...
const MainDiv = styled.div`
    ...
    .left, .right, .bottom {
        display : ${props => props.theme.button === true ? 'flex' : 'none'};
        opacity : 0;
        transition : all 0.5s;
    }
...
type MyImageSlideProps = {
    ...
    setBannerColor ?: any;
    text ?: boolean;
    onClick ?: (event ?: React.MouseEvent<HTMLParagraphElement, MouseEvent> | React.MouseEvent<HTMLImageElement, MouseEvent>) => void;
}

const MyImageSlide = ({onClick, text, setBannerColor, button = true, loading, interval = true, intervalTime = 2000, imageArray, draggable = true, keyDown = true, left = "◀︎", right = "▶"} : MyImageSlideProps) => {
    ...
    const onKeyDownImage = useCallback((e : React.KeyboardEvent<HTMLImageElement>) => {
        if(keyDown) {
            switch(e.key) {
                case 'ArrowLeft' : 
                    onClickLeft();
    ...
    },[nowImage, animationScale, button, onClick])
    // 이미지 변경 css nowImage => 이미지 번호, animationScale => 이미지 스케일 애니메이션

    return (
        <MainDiv theme={themeMemo} onKeyDown={onKeyDownImage}>
